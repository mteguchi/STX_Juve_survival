...
n.ages <- 3   # the number of age groups, given the max above. 
...
After running all the four models, we can compare LOOIC values. 
```{r}
...
When the analysis has been done for the specified maximum age and the number of age groups, load the results.
```{r}
if (file.exists(out.filename)){
...
All k values are unacceptably high, indicating incorrect model specification or outliers.  Not very encouraging... maybe a longer dataset may alleviate this problem. We can look at which data points are problematic by plotting the loo results.
