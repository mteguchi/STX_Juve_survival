...
Some k values are unacceptably high, indicating incorrect model specification or outliers. For some max_age-n_ages combinations, all k statistics are > 0.7. Not very encouraging... maybe a longer dataset may alleviate this problem. We can look at which data points are problematic by plotting the loo results.

```{r}
# this is not quite working... if the runs were from a previous analysis, i.e., file.exist() is TRUE, it needs to be loos$q_phit. If models were run now, then it should be loos[looic == min(loos)]
if (file.exists(out.filename)){
  plot(loos$q_phit)  
} else {
...
Seems like q[t,1] or q[t,2] may have higher mean values... maybe good to look at younger maturation ages? 
